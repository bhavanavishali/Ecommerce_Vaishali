    ...
    const itemLines = cart.items
      .map((item) => {
        ...
        const extras = [size && `Size: ${size}`, color && `Color: ${color}`]
          .filter(Boolean)
          .join(", ")
        const productLink = `${BASE_URL}/product/${item.product.id}`
        const description = item.product?.description || ""
        const category = item.product?.category || ""
        const imageLink = item.primary_image || ""
        
        let itemText = `• *${item.product.name}*\n`
        itemText += `  💰 Price: ${price}\n`
        itemText += `  📦 Quantity: ${item.quantity}\n`
        if (extras) itemText += `  📏 ${extras}\n`
        if (category) itemText += `  🏷️ Category: ${category}\n`
        if (description) itemText += `  📝 ${description.substring(0, 150)}${description.length > 150 ? '...' : ''}\n`
        itemText += `  🔗 View Product: ${productLink}`
          ...
          modal: {
            ondismiss: () => {
              ...
              handlePaymentFailure(
                order.id,
                "Payment Cancelled",
                "The payment was cancelled. You can view your order details or try again.",
              )
            },
          },
        }
    ...
    return (
      <Dialog open={showPaymentFailedModal} onOpenChange={setShowPaymentFailedModal}>
        <DialogContent className="max-w-md" style={{ backgroundColor: '#FFFFFF' }}>
          <DialogHeader>
            <DialogTitle className="text-xl font-semibold flex items-center" style={{ color: '#C53030' }}>
    ...
    return (
      <Dialog open={showCouponsDialog} onOpenChange={setShowCouponsDialog}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto" style={{ backgroundColor: '#FFFFFF' }}>
          <DialogHeader>
            <DialogTitle className="text-xl font-semibold flex items-center" style={{ color: '#1E2C24' }}>
          ...
          <div className="space-y-4 max-h-[60vh] overflow-y-auto">
            {couponLoading ? (
              <div className="flex items-center justify-center py-8">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2" style={{ borderColor: '#0B3D2E' }}></div>
              ...
              availableCoupons.map((coupon) => (
                <Card key={coupon.id} className="border-l-4" style={{ borderLeftColor: '#0B3D2E' }}>
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#FCF8F1' }}>
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        <div className="mb-8">
          <Link
            to="/cart"
            className="inline-flex items-center transition-colors mb-4 group"
            style={{ color: '#4B4B4B' }}
          >
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl sm:text-4xl font-bold mb-2" style={{ color: '#1E2C24' }}>Secure Checkout</h1>
              <p style={{ color: '#4B4B4B' }}>Complete your purchase with confidence</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <Card className="shadow-lg bg-white" style={{ border: '1px solid #E8DFC6' }}>
              <CardHeader className="pb-4">
                <CardTitle className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex items-center">
                    <div className="text-white rounded-full p-2 mr-3" style={{ backgroundColor: '#0B3D2E' }}>
                      <MapPin className="h-5 w-5" />
                    </div>
                    <span className="text-xl font-semibold" style={{ color: '#1E2C24' }}>Shipping Address (Optional)</span>
                  </div>
                  <Button
                    variant="outline"
...
<p className="text-center py-8" style={{ color: "#4B4B4B" }}>No addresses added yet. Adding a shipping address is optional.</p>
                  </div>
                ) : (
                  <RadioGroup
            ...
            <Card className="shadow-lg bg-white" style={{ border: '1px solid #E8DFC6' }}>
              <CardHeader className="pb-4">
                <CardTitle className="flex items-center">
                  ...
                  <span className="text-xl font-semibold" style={{ color: '#1E2C24' }}>Payment Method</span>
                </CardTitle>
              </CardHeader>
              <CardContent>
                {/* Direct Purchase — only available option */}
                <div className="border-2 rounded-xl p-5 shadow-sm" style={{ borderColor: '#0B3D2E', backgroundColor: '#F7F3EB' }}>
                  <div className="flex items-start gap-4">
                    <div className="mt-0.5 flex-shrink-0">
                      <div className="w-5 h-5 rounded-full border-2 flex items-center justify-center" style={{ borderColor: '#0B3D2E' }}>
                        <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: '#0B3D2E' }} />
                      </div>
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <svg viewBox="0 0 32 32" fill="#0B3D2E" className="h-5 w-5 flex-shrink-0" aria-hidden="true">
                          <path d="M16.003 2.667C8.636 2.667 2.667 8.636 2.667 16c0 2.348.636 4.636 1.845 6.636L2.667 29.333l6.909-1.812A13.31 13.31 0 0 0 16.003 29.333c7.367 0 13.33-5.967 13.33-13.333 0-7.364-5.963-13.333-13.33-13.333zm0 24.4a11.08 11.08 0 0 1-5.636-1.536l-.403-.24-4.1 1.076 1.094-4.003-.263-.412A11.067 11.067 0 0 1 4.933 16c0-6.107 4.963-11.067 11.07-11.067S27.067 9.893 27.067 16c0 6.11-4.96 11.067-11.064 11.067zm6.073-8.294c-.332-.167-1.966-.97-2.27-1.08-.303-.112-.524-.168-.745.167-.22.335-.854 1.08-1.048 1.302-.193.22-.386.248-.718.083-.332-.167-1.4-.515-2.666-1.643-.984-.878-1.647-1.963-1.84-2.295-.193-.332-.021-.512.146-.677.15-.148.332-.387.498-.58.167-.193.222-.332.333-.553.112-.22.056-.415-.028-.58-.083-.167-.745-1.797-1.02-2.462-.27-.647-.545-.56-.745-.57l-.635-.012c-.22 0-.58.083-.883.415-.304.332-1.158 1.133-1.158 2.763s1.186 3.205 1.351 3.428c.166.22 2.333 3.56 5.653 4.994.79.34 1.406.543 1.887.695.793.251 1.515.216 2.085.131.636-.094 1.966-.804 2.243-1.581.277-.777.277-1.44.193-1.58-.083-.14-.304-.222-.636-.39z"/>
                        </svg>
                        <span className="font-semibold text-base" style={{ color: '#1E2C24' }}>Direct Purchase via WhatsApp</span>
                        <span className="ml-auto text-xs font-semibold text-white px-2 py-0.5 rounded-full" style={{ backgroundColor: '#D4AF37' }}>
                          Only Available
                        </span>
                      </div>
                      ...
                      <p className="text-xs mt-2" style={{ color: '#9CA3AF' }}>
                        Business number: <span className="font-medium" style={{ color: '#4B4B4B' }}>+44 7553 387651</span>
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {/* Important Notice */}
                <div className="mt-4 p-4 rounded-lg" style={{ backgroundColor: '#FFF5F5', border: '1px solid #FED7D7' }}>
                  <p className="text-sm font-semibold mb-2" style={{ color: '#C53030' }}>
                    Important Notice:
                  </p>
                  <ul className="text-xs space-y-1" style={{ color: '#4B4B4B' }}>
                    <li>• No Cash on Delivery (COD) available</li>
                    <li>• An unboxing video is required for all return or damage claims</li>
                    <li>• Claims without an unboxing video will not be accepted</li>
                  </ul>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="lg:col-span-1">
            <Card className="shadow-xl bg-white sticky top-6" style={{ border: '1px solid #E8DFC6' }}>
              <CardHeader className="pb-4">
                <CardTitle className="text-xl font-semibold" style={{ color: '#1E2C24' }}>Order Summary</CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                    ...
                    cart.items.map((item) => (
                      <div key={item.id} className="space-y-3">
                        <div className="flex gap-3">
                          <div className="w-16 h-16 rounded-lg overflow-hidden flex-shrink-0" style={{ backgroundColor: '#F7F3EB' }}>
                            <img
                              src={
                                item.primary_image
                          ...
                          <div className="flex-1 min-w-0">
                            <h3 className="text-sm font-medium truncate" style={{ color: '#1E2C24' }}>{item.product.name}</h3>
                ...
                <div className="space-y-3">
                  <div className="flex gap-2">
                    ...
                    {couponApplied ? (
                      ...
                      <Button
                        ...
                        className="bg-[#0B3D2E] hover:bg-[#0B3D2E] shrink-0"
                      >
                        Apply
                      </Button>
                    )}
                  ...
                  <Button
                    ...
                    className="w-full border-[#0B3D2E] text-[#0B3D2E] hover:bg-[#0B3D2E] hover:text-white"
                  >
                    <Gift className="h-4 w-4 mr-2" />
                    View Available Coupons
                  </Button>

                  {couponError && <p className="text-xs" style={{ color: '#C53030' }}>{couponError}</p>}
                  {couponApplied && (
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <Separator />
                  <div className="flex justify-between text-lg font-bold text-[#15c753]">
                    <span>Total</span>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-3 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold text-base py-4 px-6 transition-all duration-200 shadow-sm hover:shadow-md"
                  style={{
                    backgroundColor: '#087d33',
                  }}
                  onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#1ebe5d'}
                  onMouseLeave={(e) => e.currentTarget.style.backgroundColor = '#087d33'}
                  onMouseDown={(e) => e.currentTarget.style.backgroundColor = '#19a852'}
                  onMouseUp={(e) => e.currentTarget.style.backgroundColor = '#1ebe5d'}
                >
