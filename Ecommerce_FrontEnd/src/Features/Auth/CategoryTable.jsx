...
function CategoryTable() {
  ...
  useEffect(() => {
    console.log('useEffect triggered - Page:', currentPage, 'Search:', searchTerm, 'Categories Length:', categories.length)
    fetchCategories()
  }, [currentPage, searchTerm])

  const fetchCategories = async () => {
    setLoading(true)
    console.log('Fetching categories - Page:', currentPage, 'Search:', searchTerm, 'URL:', `productapp/categories/?page=${currentPage}&search=${encodeURIComponent(searchTerm)}&per_page=7`)
    try {
      const response = await api.get(
        `productapp/categories/?page=${currentPage}&search=${encodeURIComponent(searchTerm)}&per_page=7`
      )
      console.log('API Response:', JSON.stringify(response.data, null, 2))
      const results = response.data.results || []
      const totalItems = response.data.pagination?.total_items || 0
      const perPageResponse = response.data.pagination?.per_page || 7
      const totalPagesFromResponse = response.data.pagination?.total_pages || Math.ceil(totalItems / perPageResponse)
      console.log('Parsed - Results Length:', results.length, 'Total Items:', totalItems, 'Per Page:', perPageResponse, 'Total Pages:', totalPagesFromResponse)
      ...
      setLoading(false)
      console.log('Loading set to false, Categories:', categories.length, 'Current Page:', currentPage, 'Total Pages:', totalPages)
    }
  }
  ...
  const handlePreviousPage = () => {
    if (currentPage > 1) {
      console.log('Previous button clicked - Current Page:', currentPage, 'New Page:', currentPage - 1, 'Disabled:', currentPage === 1)
      setCurrentPage(currentPage - 1)
    } else {
      console.log('Previous button disabled - Already on page 1')
    }
  }

  const handleNextPage = () => {
    if (currentPage < totalPages) {
      console.log('Next button clicked - Current Page:', currentPage, 'New Page:', currentPage + 1, 'Total Pages:', totalPages, 'Disabled:', currentPage === totalPages || totalPages === 0)
      setCurrentPage(currentPage + 1)
    } else {
      console.log('Next button disabled - Current Page:', currentPage, 'Total Pages:', totalPages)
    }
  }
          ...
          {loading ? (
            ...
            <>
                    ...
                    {categories.length > 0 ? (
                      ...
                      <TableRow>
                        <TableCell colSpan={8} className="text-center py-4">
                          {totalPages > 1 ? `No categories found on page ${currentPage}` : "No categories found"}
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
              ...
              {/* <div className="text-center mt-2 text-sm text-gray-600">
                Page {currentPage} of {totalPages} (Total Categories: {categories.length}, Items per Page: {perPage})
              </div> */}
