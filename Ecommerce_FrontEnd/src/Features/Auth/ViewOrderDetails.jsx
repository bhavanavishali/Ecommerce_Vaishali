...
//     setTimeout(() => {
//       setIsRetrying(false)
//       // Implement actual payment retry logic here
//     }, 2000)
//   }
...
//           <a href="/" className="flex items-center hover:text-[#7a2828] transition-colors duration-200">
//             <Home className="h-4 w-4 mr-1" />
//             Home
//           </a>
//           <ChevronRight className="h-4 w-4 mx-2 flex-shrink-0 text-gray-400" />
//           <a href="/my-account" className="hover:text-[#7a2828] transition-colors duration-200 whitespace-nowrap">
//             My Account
//           </a>
//           <ChevronRight className="h-4 w-4 mx-2 flex-shrink-0 text-gray-400" />
//           <a href="/my-orders" className="hover:text-[#7a2828] transition-colors duration-200 whitespace-nowrap">
//             My Orders
//           </a>
//           <ChevronRight className="h-4 w-4 mx-2 flex-shrink-0 text-gray-400" />
//           <span className="text-[#7a2828] font-semibold whitespace-nowrap">Order #{order.order_number}</span>
//         </nav>
...
//                     Your payment for this order is pending. Please complete the payment to proceed with your order
//                     processing.
//                   </p>
//                   <Button
...
//                         <p
//                           className={`mt-3 font-semibold text-sm ${
//                             order.status === "delivered" ? "text-gray-800 group-hover:text-[#7a2828]" : "text-gray-500"
//                           } transition-colors duration-200`}
//                         >
//                           Delivered
//                         </p>
...
//             <div className="mt-10 bg-gradient-to-r from-[#f8f3f3] to-[#f7f2e9] border border-[#f3e9e9] rounded-xl p-6 flex items-start gap-4 hover:shadow-md transition-all duration-300 group">
//               <div className="bg-gradient-to-r from-[#7a2828] to-[#b8860b] p-3 rounded-full shadow-md group-hover:shadow-lg group-hover:scale-110 transition-all duration-300">
//                 {getStatusIcon(order.status)}
//               </div>
//               <div>
//                 <h4 className="font-semibold text-lg bg-clip-text text-transparent bg-gradient-to-r from-[#5e1f1f] to-[#8f6608]">
//                   {order.status === "delivered"
//                     ? "Order Delivered"
//                     : order.status === "shipped"
//                       ? "Order Shipped"
...
//                     : order.status === "shipped"
//                       ? "Your order has been shipped and is on its way to you. Track your package for updates."
//                       : order.status === "processing"
//                         ? "Your order is currently being processed and prepared for shipping."
//                         : order.status === "cancelled"
...
//                                   size="sm"
//                                   disabled
//                                   className="bg-gray-300 text-gray-500 cursor-not-allowed px-4 py-2"
//                                 >
//                                   <XCircle className="h-5 w-5 mr-2" />
//                                   {item.status === "active" ? "Cancel Item" : "Cannot Cancel/Return"}
//                                 </Button>
//                               </TooltipTrigger>
//                               <TooltipContent className="bg-gradient-to-r from-[#7a2828] to-[#b8860b] text-white border-none shadow-lg">
//                                 <p>
//                                   This item cannot be {item.status === "active" ? "cancelled" : "cancelled or returned"}{" "}
//                                   as it is {item.status}.
//                                 </p>
//                               </TooltipContent>
//                             </Tooltip>
//                           </TooltipProvider>
...
//                         <div className="mt-3 text-red-600 bg-red-50 p-3 rounded-md text-sm border border-red-100 shadow-sm">
//                           <span className="font-medium">Cancellation Reason:</span> {item.cancel_reason}
//                         </div>
//                       )}
//                       {item.status === "return_requested" && item.return_reason && (
//                         <div className="mt-3 text-amber-600 bg-amber-50 p-3 rounded-md text-sm border border-amber-100 shadow-sm">
//                           <span className="font-medium">Return Reason:</span> {item.return_reason}
//                         </div>
//                       )}
//                       {item.status === "returned" && item.return_reason && (
//                         <div className="mt-3 text-emerald-600 bg-emerald-50 p-3 rounded-md text-sm border border-emerald-100 shadow-sm">
//                           <span className="font-medium">Return Approved:</span> {item.return_reason}
//                         </div>
//                       )}
//                       {item.status === "return_denied" && item.return_reason && (
//                         <div className="mt-3 text-red-600 bg-red-50 p-3 rounded-md text-sm border border-red-100 shadow-sm">
//                           <span className="font-medium">Return Denied:</span> {item.return_reason}
//                         </div>
//                       )}
//                     </div>
//                   </div>
...
//                     <div className="flex justify-between text-gray-700 group hover:bg-white hover:shadow-sm p-3 rounded-lg transition-all duration-200">
//                       <span>Subtotal</span>
//                       <span className="font-semibold group-hover:text-[#7a2828] transition-colors duration-200">
//                         ₹{order.total_amount}
//                       </span>
//                     </div>
//                     <div className="flex justify-between text-gray-700 group hover:bg-white hover:shadow-sm p-3 rounded-lg transition-all duration-200">
//                       <span>Discount</span>
//                       <span className="text-emerald-600 font-semibold group-hover:scale-105 transition-transform duration-200">
...
//                       </span>
//                     </div>
//                     <Separator className="bg-gradient-to-r from-[#e5d1d1] to-[#f0e6c9] h-0.5 rounded-full" />
//                     <div className="flex justify-between font-bold text-lg p-3 bg-white rounded-lg shadow-sm">
//                       <span>Total</span>
//                       <span className="bg-clip-text text-transparent bg-gradient-to-r from-[#7a2828] to-[#b8860b]">
//                         ₹{order.final_total}
//                       </span>
...
//                     <div className="flex items-start gap-3 group hover:bg-white p-3 rounded-lg transition-all duration-200 hover:shadow-sm">
//                       <User className="h-5 w-5 text-[#7a2828] mt-0.5 flex-shrink-0" />
//                       <div>
//                         <p className="text-sm text-gray-500">Name</p>
//                         <p className="font-medium text-gray-800 group-hover:text-[#5e1f1f] transition-colors duration-200">
...
//                     <div className="flex items-start gap-3 group hover:bg-white p-3 rounded-lg transition-all duration-200 hover:shadow-sm">
//                       <Phone className="h-5 w-5 text-[#7a2828] mt-0.5 flex-shrink-0" />
//                       <div>
//                         <p className="text-sm text-gray-500">Phone</p>
//                         <p className="font-medium text-gray-800 group-hover:text-[#5e1f1f] transition-colors duration-200">
//                           {order.order_address.phone || "+91 9876543210"}
//                         </p>
//                       </div>
//                     </div>

//                     <div className="flex items-start gap-3 group hover:bg-white p-3 rounded-lg transition-all duration-200 hover:shadow-sm">
//                       <Mail className="h-5 w-5 text-[#7a2828] mt-0.5 flex-shrink-0" />
//                       <div>
//                         <p className="text-sm text-gray-500">Email</p>
//                         <p className="font-medium text-gray-800 group-hover:text-[#5e1f1f] transition-colors duration-200">
//                           {order.user_email || "customer@example.com"}
//                         </p>
//                       </div>
//                     </div>

//                     <div className="flex items-start gap-3 group hover:bg-white p-3 rounded-lg transition-all duration-200 hover:shadow-sm">
//                       <Calendar className="h-5 w-5 text-[#7a2828] mt-0.5 flex-shrink-0" />
//                       <div>
...
//                     </div>
//                     <h2 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-[#7a2828] to-[#b8860b]">
//                       Payment Method
//                     </h2>
//                   </div>
//                   <div className="text-gray-700 flex items-center gap-3 p-4 border border-[#e5d1d1] rounded-xl hover:bg-white hover:shadow-md transition-all duration-300 group">
//                     <div className="h-12 w-12 bg-gradient-to-r from-[#7a2828] to-[#b8860b] rounded-full flex items-center justify-center shadow-md group-hover:shadow-lg group-hover:scale-110 transition-all duration-300">
//                       <svg
//                         className="h-6 w-6 text-white"
//                         viewBox="0 0 24 24"
//                         fill="none"
...
//                       >
//                         <path
//                           d="M17 9V7C17 4.23858 14.7614 2 12 2C9.23858 2 7 4.23858 7 7V9M5 9C4.44772 9 4 9.44772 4 10V20C4 20.5523 4.44772 21 5 21H19C19.5523 21 20 20.5523 20 20V10C20 9.44772 19.5523 9 19 9H5Z"
//                           stroke="currentColor"
//                           strokeWidth="2"
//                           strokeLinecap="round"
...
//                     </h2>
//                   </div>
//                   <div className="text-gray-700 space-y-2 p-4 border border-[#e5d1d1] rounded-xl hover:bg-white hover:shadow-md transition-all duration-300 group">
//                     <p className="font-semibold group-hover:text-[#7a2828] transition-colors duration-200">
//                       {order.order_address.name}
//                     </p>
//                     <p className="group-hover:text-gray-800 transition-colors duration-200">
...
const ViewOrderDetails = () => {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        const response = await api.get(`cartapp/orders/${id}/`);
        setOrder(response.data);
        console.log("boooooooooooooooooo",response.data)
        setIsLoading(false);
      } catch (err) {
        setError("Failed to load order details. Please try again later.");
        setIsLoading(false);
  ...
  //       text: "Invoice can only be downloaded after the order is delivered.",
  //     });
  //     return;
  //   }
  //   navigate(`/invoice`);
