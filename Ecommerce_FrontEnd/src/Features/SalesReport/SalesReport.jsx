...
const SalesReport = () => {
  ...
  const [filterType, setFilterType] = useState("thisYear"); // Default to "thisYear"
  const [salesData, setSalesData] = useState([]);
  const [summary, setSummary] = useState({
    totalSales: 0,
    totalDiscount: 0,
  ...
  const handleDownloadCSV = () => {
    const headers = [
      'Order ID', 'Date', 'Order MRP', 'Item Discount', 'Order Subtotal',
      'Coupon Discount', 'Shipping Charge', 'Total Amount', 'Payment Method',
    ];
    const rows = salesData.map(row => [
      row.orderId,
      row.date,
  ...
  const handleDownloadPDF = () => {
    const doc = new jsPDF();
    doc.text(`Sales Report (${filterType})`, 14, 20);
    doc.autoTable({
      startY: 30,
      head: [[
        'Order ID', 'Date', 'Order MRP', 'Item Discount', 'Order Subtotal',
        'Coupon Discount', 'Shipping', 'Total Amount', 'Payment Method',
      ]],
      body: salesData.map(row => [
        row.orderId,
        row.date,
